...
def main():
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        adjust_learning_rate(opt, model.optimizer, epoch)  #放入lr_schedules 更新学习率
...
def train(opt, train_loader, model, epoch):
    ...
    if epoch == 0:
        ...
        logger.info('criterion trainable parameters: {}M'.format(count_params(model.hal_loss)))
    ...
    for i, train_data in enumerate(train_loader):
        ...
        #print("Model+backward time:", time.time() - start)
        #start = time.time()
...
def validate(opt, val_loader, model):
    ...
    with torch.no_grad():
        ...
        img_embs, cap_embs = encode_data(model, val_loader, opt.log_step, logging.info,backbone=opt.precomp_enc_type == 'backbone')
