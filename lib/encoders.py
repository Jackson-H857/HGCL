...
class VisionTransEncoder(nn.Module):
    def __init__(self, opt, embed_size=1024):
        ...
        if 'swin' in opt.vit_type:                           
            ...
            self.hidden_size = self.visual_encoder.config.hidden_size  # 从config获取
    ...
    def forward(self, images, image_lengths, graph=False):
        ...
        # patch_features = self.vision_proj(img_feats)
    ...
    torch.cat([img_emb_res, img_emb], dim=-1)))

        img_emb = beta * img_emb_res + (1 - beta) * img_emb
...
class EncoderText_BERT(nn.Module):
    ...
    def forward(self, x, lengths, graph=False):
        ...
        beta = torch.sigmoid(self.gate(
            torch.cat([cap_emb_res, cap_emb], dim=-1)))

        cap_emb = beta * cap_emb_res + (1 - beta) * cap_emb
