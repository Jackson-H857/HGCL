...
class VSEModel(nn.Module):   #object
    def __init__(self, opt, eval=False):
        ...
        self.hal_loss = HyHCL(opt=opt)
        #self.hal_loss = ContrastiveLoss(opt=opt)
        print(self.hal_loss)
        ...
        if opt.precomp_enc_type == 'basic':
            if self.opt.optim == 'adam':
                ...
                self.optimizer = torch.optim.AdamW([
                    ...
                    {'params': self.img_enc.parameters(), 'lr': opt.learning_rate*0.1}  #0.01  laps 是0.1
    ...
    def train_emb(self, images, captions, lengths, image_lengths=None,img_ids=None):
        ...
        # if self.Eiters == self.opt.warmup:
        #     logger.info("Warmup finished, unfreezing backbone...")
