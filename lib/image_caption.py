...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, opt, data_path, split, tokenizer, train):
        ...
        # coco: 119287 imgs, 
        loc = os.path.join(opt.data_path, opt.dataset ,'precomp')  ####
        ...
        with open(os.path.join(loc, '%s_caps.txt' % self.split), 'r',encoding='utf-8') as f:
            for line in f:
...
def collate_fn_ours(data):
    ...
    images = torch.stack(images, 0)
    img_lengths = torch.tensor([image.size(0) for image in images])
    ...
    return images,img_lengths, targets, lengths, ids, img_ids, repeat


def get_loader(data_path, data_split, tokenizer,opt, batch_size=128,
    ...
    dset =  RawImageDataset(opt, data_path, data_split, tokenizer, train)
    ...
    if opt.multi_gpu and train:
        ...
        sampler = torch.utils.data.DistributedSampler(dset, num_replicas=num_tasks, rank=global_rank, shuffle=shuffle)
        shuffle = False
    else:
        sampler = None
